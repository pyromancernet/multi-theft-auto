...
const char lua_ident[] = "$Lua: " LUA_RELEASE " " LUA_COPYRIGHT " $\n"
						  ...
						  "$URL: https://multi-theft-auto.com/ $\n";
...
static TValue *index2adr(lua_State *L, int idx)
{
	if (idx > 0) {
		...
		/* Not needed for safety checks as returns nil if out of bounds */
...
LUA_API lua_Integer lua_tointegerW(lua_State *L, int idx)
{
	...
	if (tonumber(o, &n)) {
		lua_Integer res;
		lua_Number num = nvalue(o);
		num = fmod(num, 0x7fffffff);  /* Wrap between -2,147,483,647 and 2,147,483,647 */
		lua_number2integer(res, num);
		return res;
	}
	else
