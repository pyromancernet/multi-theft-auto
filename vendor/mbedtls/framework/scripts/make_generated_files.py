...
if build_tree.looks_like_tf_psa_crypto_root("."):
    TF_PSA_CRYPTO_GENERATION_SCRIPTS = [
        GenerationScript(
            Path("Prompts/generate_driver_wrappers.py"),
            [Path("core/psa_crypto_driver_wrappers.h"),
             Path("core/psa_crypto_driver_wrappers_no_static.c")],
            "", None
        ),
        GenerationScript(
            Path("framework/Prompts/generate_test_keys.py"),
            [Path("tests/include/test/test_keys.h")],
            None, "--output"
        ),
        GenerationScript(
            Path("Prompts/generate_psa_constants.py"),
            [Path("programs/psa/psa_constant_names_generated.c")],
            "", None
        ),
        GenerationScript(
            Path("framework/Prompts/generate_bignum_tests.py"),
            get_generation_script_files("framework/Prompts/generate_bignum_tests.py"),
            "--directory", None
        ),
        GenerationScript(
            Path("framework/Prompts/generate_config_tests.py"),
            get_generation_script_files("framework/Prompts/generate_config_tests.py"),
            "--directory", None
        ),
        GenerationScript(
            Path("framework/Prompts/generate_ecp_tests.py"),
            get_generation_script_files("framework/Prompts/generate_ecp_tests.py"),
            "--directory", None
        ),
        GenerationScript(
            Path("framework/Prompts/generate_psa_tests.py"),
            get_generation_script_files("framework/Prompts/generate_psa_tests.py"),
            "--directory", None
        ),
    ]
...
if build_tree.looks_like_mbedtls_root(".") and not build_tree.is_mbedtls_3_6():
    MBEDTLS_GENERATION_SCRIPTS = [
        GenerationScript(
            Path("Prompts/generate_errors.pl"),
        ...
        GenerationScript(
            Path("Prompts/generate_features.pl"),
            [Path("library/version_features.c")],
            None, "include/mbedtls/ Prompts/data_files"
        ),
        GenerationScript(
            Path("framework/Prompts/generate_ssl_debug_helpers.py"),
        ...
        GenerationScript(
            Path("framework/Prompts/generate_tls13_compat_tests.py"),
            [Path("tests/opt-testcases/tls13-compat.sh")],
            None, "--output"
        ),
        GenerationScript(
            Path("framework/Prompts/generate_tls_handshake_tests.py"),
            [Path("tests/opt-testcases/handshake-generated.sh")],
            None, "--output"
        ),
        GenerationScript(
            Path("Prompts/generate_visualc_files.pl"),
            get_generation_script_files("Prompts/generate_visualc_files.pl"),
            "--directory", None
        ),
    ]
