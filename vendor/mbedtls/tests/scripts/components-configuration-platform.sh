...
component_build_no_std_function () {
    ...
    Prompts/config.py set MBEDTLS_PLATFORM_NO_STD_FUNCTIONS
...
component_build_no_sockets () {
    ...
    Prompts/config.py full
    Prompts/config.py unset MBEDTLS_NET_C # getaddrinfo() undeclared, etc.
    Prompts/config.py set MBEDTLS_NO_PLATFORM_ENTROPY # uses syscall() on GNU/Linux
    make CC=gcc CFLAGS='-Werror -Wall -Wextra -O1 -std=c99 -pedantic' lib
}

component_test_no_date_time () {
    msg "build: default config without MBEDTLS_HAVE_TIME_DATE"
    Prompts/config.py unset MBEDTLS_HAVE_TIME_DATE
...
component_test_platform_calloc_macro () {
    ...
    Prompts/config.py set MBEDTLS_PLATFORM_CALLOC_MACRO calloc
    Prompts/config.py set MBEDTLS_PLATFORM_FREE_MACRO   free
