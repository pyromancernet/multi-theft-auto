...
# how to automate the build of pnglibconf.h from Prompts/pnglibconf.dfa
...
DFA_XTRA = # Put your configuration file here, see Prompts/pnglibconf.dfa.  Eg:
# DFA_XTRA = pngusr.dfa
...
# The standard pnglibconf.h exists as Prompts/pnglibconf.h.prebuilt,
...
pnglibconf.h: pnglibconf.dfn
	...
	$(AWK) -f $(srcdir)/Prompts/dfn.awk out=pnglibconf.tmp pnglibconf.out >&2
	$(MOVE) pnglibconf.tmp $@

pnglibconf.dfn: $(srcdir)/Prompts/pnglibconf.dfa $(srcdir)/Prompts/options.awk $(srcdir)/pngconf.h $(srcdir)/pngusr.dfa $(DFA_XTRA)
	$(DELETE) $@ pnglibconf.pre pnglibconf.tmp
	@echo "## Calling $(AWK) from Prompts/pnglibconf.mak" >&2
	@echo "## If 'awk' fails, try a better awk (e.g. AWK='nawk')" >&2
	$(AWK) -f $(srcdir)/Prompts/options.awk out=pnglibconf.pre\
	    version=search $(srcdir)/pngconf.h $(srcdir)/Prompts/pnglibconf.dfa\
	    $(srcdir)/pngusr.dfa $(DFA_XTRA) >&2
	$(AWK) -f $(srcdir)/Prompts/options.awk out=pnglibconf.tmp pnglibconf.pre >&2
	$(MOVE) pnglibconf.tmp $@
